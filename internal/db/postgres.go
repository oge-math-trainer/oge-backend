...
func (s *Store) CreatePasswordResetCode(ctx context.Context, email, codeHash string, expiresAt time.Time) (bool, error) {
	tx, err := s.pool.Begin(ctx)
	if err != nil {
	...
	user, err := selectUserByEmail(ctx, tx, email)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, nil
	}
	if err != nil {
	...
	if _, err := tx.Exec(ctx, `
		...
		VALUES ($1, $2, $3)
	`, user.ID, codeHash, expiresAt); err != nil {
		return false, app.Internal(err)
	}
...
func (s *Store) ResetPasswordWithCode(ctx context.Context, email, codeHash, passwordHash string, now time.Time) (auth.User, error) {
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return auth.User{}, app.Internal(err)
	}
	...
	row := tx.QueryRow(ctx, `
		SELECT u.id, u.email, u.created_at, prc.id
	...
	var user auth.User
	var resetCodeID int64
	if err := row.Scan(&user.ID, &user.Email, &user.CreatedAt, &resetCodeID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return auth.User{}, app.Unauthorized("Invalid or expired password reset code")
		}
		return auth.User{}, app.Internal(err)
	}
